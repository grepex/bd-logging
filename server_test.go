...
func Test_requestLogger(t *testing.T) {
	...
	if logBuffer.String() != expectedLogString {
		t.Errorf("Test for log string failed: expected '%s' but got '%s'", expectedLogString, logBuffer.String())
	}

	// Compare rr.Code to the expected status code.
	if rr.Code != expectedStatusCode {
		t.Errorf("Test for status code failed: expected '%d' but got '%d'", expectedStatusCode, rr.Code)
	}
}
